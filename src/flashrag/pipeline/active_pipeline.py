...
class ReCallPipeline(BasicPipeline):
    ...
    def extract_last_assistant_response(self, text: str) -> str:
        ...
        # if pos != -1:
        #     cleaned_response = text[pos + len("</think>") :]
        # else:
        #     cleaned_response = text
        
        cleaned_response = re.findall(r'<answer>(.*?)</answer>', text, re.DOTALL)
        cleaned_response = cleaned_response[-1].strip() if cleaned_response else ''

        return cleaned_response.strip()

    def run_item(self, item):
        if item.prefix:
            response = self.re_call.run_prefix(env=wikipedia_search_env.replace('<search-url-placeholder>', self.config["remote_retriever_url"]), func_schemas=wikipedia_search_schemas, question=item.question, prefix = item.prefix)
        elif item.hint:
            response = self.re_call.run_hint(env=wikipedia_search_env.replace('<search-url-placeholder>', self.config["remote_retriever_url"]), func_schemas=wikipedia_search_schemas, question=item.question, hint=item.hint)
        else:
