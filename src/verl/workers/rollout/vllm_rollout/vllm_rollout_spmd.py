            ...
            for step in range(self.config.max_turns):
                ...
                if step != 0 or 'prefix_loss_mask' not in prompts.batch:
                    ...
                    for i, idx in enumerate(active_indices):
                        ...
                        if finish_reason == 'stop' and (stop_reason == None or stop_reason == self.tokenizer.pad_token_id or stop_reason == '</search>'):
                            if stop_reason == '</search>':
                                output_ids = output_ids[:-1] + self.tokenizer.encode('</search>')[-1:]
